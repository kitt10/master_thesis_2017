...
transfer fcn          & sigmoid                & number of epochs               & 100               & retrain                       & True             \\ \hline
                      &                        & minibatch size                 & 1                & retraining epochs             & 10               \\ \hline
\end{tabular}}
...
transfer function       & sigmoid                 & number of epochs               & 100              \\ \hline
       &                  & batch size               & 10              \\ \hline
\end{tabular}
\caption{Settings for learning a dense feedforward net the MNIST dataset.}
\label{tab:examples:mnist_training_settings}
\end{table}

The training results are summarized in \cref{tab:examples:mnist_training_results}. A confusion matrix for testing data is given in \cref{fig:examples:mnist_cm}.
...
\caption{MNIST: confusion matrix (testing data).}
\label{fig:examples:mnist_cm}
\end{figure}

The rest of this section is devoted to analysis of the pruning method on MNIST database. Parameters of the learning-pruning procedure are listed in \cref{tab:examples:mnist_settings}.
...
\caption{Illustration of the pruning procedure applied on MNIST dataset (selected observation). Required accuracy: $ 96\% $.}
...
\end{figure}
 ...
 \cref{fig:examples:mnist_pruning_vs_req_acc} gives the statistics by running 10 observations of the pruning procedure for several values of required classification accuracy. We observed the number of synapses (blue axis) and used features after pruning (red axis).
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{pruned_net_mnist_05.eps}
\caption{Result of network pruning and pathing, MNIST data, accuracy: $ 50\% $.}
\label{fig:examples:pruned_net_mnist_05}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{pruned_net_mnist_097.eps}
\caption{Result of network pruning and pathing (synapses to $ 17^{th} $ hidden neuron only), MNIST data, accuracy: $ 97\% $.}
\label{fig:examples:pruned_net_mnist_05}
\end{figure}
